"""
五星级酒店客房服务 Agent — FastAPI 生产服务器
====================================================
提供给总控 Agent (MainRouter) 调用的标准 HTTP 接口
...
from room_service_agent import invoke_agent, invoke_agent_structured, clear_session, ALL_TOOLS
...
class ChatResponse(BaseModel):
    ...
    result_type: str = Field(default="execute", description="execute / need_clarify / reject")
    final_intent: dict = Field(default_factory=dict, description="最终确定的意图")
    decision_trace: list = Field(default_factory=list, description="全链路决策追溯")
    tool_calls: list[str] = Field(default_factory=list, description="本次调用的工具名称列表")
...
async def lifespan(app: FastAPI):
    """启动 / 关闭时的钩子"""
    logger.info("=" * 50)
    logger.info("🏨 五星级酒店客房服务 Agent 启动")
    logger.info("   模型: qwen2.5:7b (Ollama)")
...
app = FastAPI(
    ...
    description="五星级酒店客房服务智能体 — 支持清扫、补给、报修、洗衣、唤醒、呼叫前台",
    version="1.0.0",
    lifespan=lifespan,
)
...
async def chat(request: ChatRequest):
    ...
    Day 10: 返回结构化 JSON（含 result_type + final_intent + decision_trace）
    """
    ...
    # 从 trace 中提取工具调用
    tool_calls = []
    for trace in structured.get("decision_trace", []):
        if isinstance(trace, dict) and trace.get("step") == "tool_executor":
            tool_calls.append(trace.get("tool_name", "unknown"))

    return ChatResponse(
        response=structured.get("response_text", ""),
        session_id=request.session_id,
        result_type=structured.get("result_type", "execute"),
        final_intent=structured.get("final_intent", {}),
